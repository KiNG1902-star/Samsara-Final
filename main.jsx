...
// WE ARE USING THE GLOBAL SUPABASE LOADED FROM INDEX.HTML
// NO "IMPORT" LINE FOR SUPABASE HERE!
...
// Access supabase from the window object (loaded via CDN)
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#060a0f', color: 'white', fontFamily: 'sans-serif', padding: '25px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '35px' }}>
        ...
        <div style={{ padding: '6px 12px', backgroundColor: '#1c1c1e', borderRadius: '10px', fontSize: '10px', color: '#00ff88' }}>VAULT LIVE</div>
      </div>
      ...
      {showDeposit && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.95)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 100 }}>
          <div style={{ backgroundColor: '#1c1c1e', padding: '30px', borderRadius: '30px', width: '100%', maxWidth: '350px', border: '1px solid #2c2c2e' }}>
            ...
            <input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} style={{ width: '100%', padding: '20px', backgroundColor: '#060a0f', border: '1px solid #2c2c2e', borderRadius: '15px', color: 'white', fontSize: '24px', fontWeight: 'bold', boxSizing: 'border-box', marginBottom: '20px' }} />
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowDeposit(false)} style={{ flex: 1, padding: '15px', borderRadius: '12px', border: 'none', backgroundColor: '#333', color: 'white' }}>CANCEL</button>
